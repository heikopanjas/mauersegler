...
- Renamed the extension identity from `vscode-swift-format`
  (displayed as "Swift Format") to `vscode-mauersegler`
  (displayed as "Mauersegler"), after the original display name
  collided with another extension's on the Marketplace.
- Repository moved to `github.com/heikopanjas/mauersegler`.
- Replaced the icon, and updated the gallery banner color to
  match it.
